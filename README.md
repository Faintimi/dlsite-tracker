...
> **跨平台桌面应用（Windows / macOS）** + 配套的合规数据管道：每日自动更新销量 / 热榜 / 分类人气，浏览时支持分类交集筛选、收藏夹与制作者追踪——**数据与封面全部只存在你本机**。
>
> *A cross-platform desktop browser & filter for DLsite doujin games (Windows / macOS), backed by a robots-compliant daily data pipeline. All data stays on your machine.*
...
管道在本机采集 DLsite 的公开商品元数据（名称 / 社团 / 价格 / 评分 / 评价数 / 销量 / 分类 / 排行榜 等），存入本机 SQLite，并导出为 JSON / CSV 与封面缩略图；配套的桌面应用（Windows / macOS 与 macOS 原生版）读取导出文件，提供浏览、筛选、收藏与一键更新。**应用自身不联网**——联网行为全部发生在管道里。

## 功能亮点

- **每日自动维护**（默认 23:30）：热榜名次 / 在榜销量 / 分类人气 / 官方人气序
- **应用内一键更新双档**：「立即更新热榜（快）」约 1–2 分钟；「完整维护（同夜间计划）」包含分类人气、封面与导入续传；**完成后自动刷新列表**
- **筛选**：分类多选取交集、评分 / 销量 / 价格 / 发售年份 / 关键词 / 收藏状态 / 仅关注作者
- **排序**：销量 / 评分 / 价格 / 发售时间 / 名称
- **收藏夹**（一个作品可属于多个）、**制作者关注**、**悬停详情卡**、**右键快捷菜单**、五种显示形式（网格卡 / 封面墙 / 底部信息栏 / 紧凑列表 / 横条）
- 顶部横幅实时显示更新 / 导入进度，完成后自动刷新

## 快速开始

环境要求：**macOS / Windows / Linux + Python ≥ 3.9**（仅标准库，零第三方依赖；无需 `pip install`）。
...
应用内「选择数据文件」→ 选择导出的 `out/works.json`。说明见 [`app/README.md`](app/README.md) / [`app/README_EN.md`](app/README_EN.md)。
...
- 应用自身不联网；「更新数据」只是在本机启动本仓库的固定任务链（`scripts/*.sh` / `python -m dlsite_tracker task …`）
...
- **Quick start**: `python3 -m dlsite_tracker update` → export `out/works.json` → open it in the desktop app (`desktop/`; Windows installer available from GitHub Actions) or the macOS app (`bash scripts/build-app.sh`).
